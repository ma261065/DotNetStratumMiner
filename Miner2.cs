...
namespace SharpMiner
{
    class Miner2
    {
        ...
        public void doScrypt(object args)
        {
            ...
            try
            {
                ...
                while (!done)
                {
                    ...
                    //scrypted = CryptSharp.Utility.SCrypt.ComputeDerivedKey(databyte, databyte, 1, 1024, 1, 4, 32);
                    try
                    {
                        try
                        {
                            scrypted = Replicon.Cryptography.SCrypt.SCrypt.DeriveKey(databyte, databyte, 1024, 1, 1, 32);
                        }
        ...
        public void Go2(BackgroundWorker bw, object args)
        {
            Array argArray = new object[3];
            argArray = (Array)args;
            string user = (string)argArray.GetValue(0);
            ...
            i++;
            string cred = user + ":" + pass;
            string resp = Program.CurrentRequest;  //getwork(url, cred);
            JobID = Program.CurrentJobID;
            ...
            while (!done)
            {
                if (bw.CancellationPending)
                {
                    Console.WriteLine("Cancelled background thread");
            ...
            if (fnonce != 0)
            {
                ...
                databyte2[77] = (byte)(fnonce >> 16);
                databyte2[76] = (byte)(fnonce >> 24);
                EndResult = HexToString(databyte2) + ":" + JobID;
                //submit(url, cred, HexToString(databyte2));
            }
        }
        ...
        {
            Console.WriteLine("Calling GETWORK");
        ...
        private void newblock(string url, string c)
        {
            ...
            try
            {
                ...
                Console.WriteLine("New Block Detected!");
            }
